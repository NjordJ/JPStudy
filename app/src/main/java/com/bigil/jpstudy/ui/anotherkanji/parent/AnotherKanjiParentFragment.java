...
public class AnotherKanjiParentFragment extends Fragment {
    ...
    //Variables
    public ArrayList<KanjiItem> kanjiAnotherItemArrayList = new ArrayList<>();

    //RecyclerView for show information
    private RecyclerView mRecyclerViewAnotherKanji;
    private BeginnerKanjiAdapter mAdapterAnotherKanji;
    private RecyclerView.LayoutManager mLayoutManagerAnotherKanji;

    public View onCreateView(@NonNull LayoutInflater inflater,
                             ViewGroup container, Bundle savedInstanceState) {
        View root = inflater.inflate(R.layout.fragment_anotherlistofkanji, container, false);
        ...
        Button buttonStartLearningAnotherKanji = root.findViewById(R.id.buttonStartLearningAnotherKanji);
        ...
        try {
            ...
            for (int i=0; i<len; i++) {
                ...
                if(Integer.valueOf("10").equals(grade)){
                    kanjiAnotherItemArrayList.add(new KanjiItem(kanji,grade,stroke_count, jsonUtils.toStringArray(meanings),heisig_en, jsonUtils.toStringArray(kun_readings),
                            jsonUtils.toStringArray(on_readings), jsonUtils.toStringArray(name_readings), jlpt,unicode, resourceId));
                }

            }

            Collections.sort(kanjiAnotherItemArrayList, new Comparator<KanjiItem>() {
                @Override
        ...
        mRecyclerViewAnotherKanji = root.findViewById(R.id.recyclerViewAnotherKanji);
        mRecyclerViewAnotherKanji.setHasFixedSize(true);
        mLayoutManagerAnotherKanji = new LinearLayoutManager(getContext());
        //mLayoutManagerBeginnerKanji = new GridLayoutManager(getActivity(),5);
        mAdapterAnotherKanji = new BeginnerKanjiAdapter(kanjiAnotherItemArrayList);

        mRecyclerViewAnotherKanji.setLayoutManager(mLayoutManagerAnotherKanji);
        mRecyclerViewAnotherKanji.setAdapter(mAdapterAnotherKanji);

        mAdapterAnotherKanji.setOnItemClickListener(new BeginnerKanjiAdapter.OnItemClickListener() {
            @Override
            public void onItemClick(int position) {
                ...
                bundle.putParcelable("KanjiItemData", kanjiAnotherItemArrayList.get(position));
        ...
        buttonStartLearningAnotherKanji.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                ...
                Bundle bundle = new Bundle();
                bundle.putParcelableArrayList("KanjiItemDataTests", kanjiAnotherItemArrayList);
